...
struct LeaderboardView: View {
    @State private var selectedCategory: LeaderboardCategory = .groups
    @State private var selectedGroup: MockGroup?
    @State private var selectedCompetition: MockGroupCompetition?
    ...
    private var groupPicker: some View {
        Picker("Select a Group", selection: $selectedGroup) {
            ...
            ForEach(mockGroups, id: \.id) { group in
                Text(group.name).tag(group as MockGroup?)
            }
        }
    ...
    private var competitionPicker: some View {
        Picker("Select a Competition", selection: $selectedCompetition) {
            Text("Select a Competition").tag(nil as MockGroupCompetition?)
            ForEach(mockCompetitions, id: \.id) { competition in
                Text(competition.competition_name).tag(competition as MockGroupCompetition?)
            }
        }
        .pickerStyle(MenuPickerStyle())
    }
    ...
    private var leaderboardChart: some View {
        Chart(members.prefix(5)) { member in
            BarMark(
    ...
    private func fetchMockLeaderboardData() {
        switch selectedCategory {
        case .groups:
            if let group = selectedGroup {
                members = mockMembers(for: group)
            }
        case .competitions:
            if let competition = selectedCompetition {
                members = mockCompetitionMembers(for: competition)
            }
        }
        members.sort { $0.points > $1.points }
    }
    
    private func mockMembers(for group: MockGroup) -> [LeaderboardMember] {
        return (1...10).map { LeaderboardMember(id: UUID(), name: "Member \($0)", points: Int.random(in: 0...1000)) }
    }
    
    private func mockCompetitionMembers(for competition: MockGroupCompetition) -> [LeaderboardMember] {
        return (1...10).map { LeaderboardMember(id: UUID(), name: "Player \($0)", groupName: "Group \($0 % 3 + 1)", points: Int.random(in: 0...1000)) }
    }
}
